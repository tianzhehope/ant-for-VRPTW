...
def get_heuristic(i, j, data, dis):
    ...
    return heuristic
#蚂蚁移动到下一节点
def get_next_node(current_node, candidate, Ph_matrix, dis, data, num_node, a, b):
    #相邻边信息素
    ...
    for i in candidate_tmp:
        if load >= data[i][3]:#满足需求
            if (dis[current_node][i] + dis[i][0])/speed + data[i][6] + current_time <= data[0][5]:#能返回仓库
                if (dis[current_node][i] / speed + current_time) <= min(data[i][5], data[0][5]):#满足硬时间窗
                    candidate.append(i)
    return candidate
#成本计算
def F(tc, dis, route_matrix):
    F = 0
...
#            add_flag = 1#超时标记，1为未超时
            if current_time > data[0][5]:#超时则寻找新解
                continue
#        for i in range(5):
#            print(current_node)
...
#                print(current_node)
                ...
                current_node = get_next_node(current_node, candidate, Ph_matrix, dis, data, num_node, a, b)
...
#        print(pareto(F_tmp, C_tmp, local_best_result_matrix))
        if pareto(F_tmp, C_tmp, local_best_result_matrix) == 1:#若为帕累托最优解则加入local_best_result_matrix
            local_best_result_matrix.append([route_matrix, trajectory, arrival_time, F_tmp, C_tmp])
        for i in range(len(local_best_result_matrix)):
    ...
    if best_result_matrix == []:
        ...
        for i in local_best_result_matrix:
            if pareto(i[3], i[4], best_result_matrix) == 1:
                best_result_matrix.append[i]
        for i in range(len(best_result_matrix)):
