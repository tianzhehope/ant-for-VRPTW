...
#信息素更新
def update_pheromone(Ph_matrix, p, T_max, T_min, tc, dis, route_matrix, f_matrix, em, cr, local_best_result_matrix, local_max_F, local_min_F, local_max_C, local_min_C, local_max_S, local_min_S):#信息素更新
    Ph_matrix = np.matrix(Ph_matrix)
    Ph_matrix = Ph_matrix*(1 - p)#信息素挥发
    for i in local_best_result_matrix:
        delta = 3 - (i[3] - local_min_F) / (local_max_F - local_min_F) - (i[4] - local_min_C) / (local_max_C - local_min_C) - (i[5] - local_min_S) / (local_max_S - local_min_S)
...
#        print(pareto(F_tmp, C_tmp, local_best_result_matrix))
    ...
    #更新信息素 *分母为零
    Ph_matrix = update_pheromone(Ph_matrix, p, T_max, T_min, tc, dis, route_matrix, f_matrix, em, cr, local_best_result_matrix, local_max_F, local_min_F, local_max_C, local_min_C, local_max_S, local_min_S)
    #更新最优解集
