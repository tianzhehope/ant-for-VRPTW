...
#读入数据
data = np.loadtxt('D:\资料\研究生\耿丽\\r201.txt').astype(np.int64)
...
#蚁群优化求解
max_t = 100#最大迭代次数
re_init = 10#n次迭代后重新初始化信息素矩阵
m = 80#蚂蚁个数
a = 1#伪随机比例行动选择规则权重因子（信息素）
b = 3#伪随机比例行动选择规则权重因子（启发式信息）
p = 0.8#挥发系数
q0 = 0.8#伪随机比例行动选择规则参数
T_max = 6.0#信息素最大值
T_min = 0.001#信息素最小值
...
def pareto(x, y, z, matrix):#1则加入
    ...
    if matrix != []:
        for i in matrix:
            if ((i[3] <= x) & (i[4] < y) & (i[5] < z)) | ((i[3] < x) & (i[4] <= y) & (i[5] < z)) | ((i[3] < x) & (i[4] < y) & (i[5] <= z)) | ((i[3] <= x) & (i[4] <= y) & (i[5] < z)) | ((i[3] < x) & (i[4] <= y) & (i[5] <= z)) | ((i[3] <= x) & (i[4] < y) & (i[5] <= z)):
                flag = 0
                break
    return flag
...
#        print(pareto(F_tmp, C_tmp, local_best_result_matrix))
    ...
    best_fee.append([t, min([i[3] for i in best_result_matrix])])
...
X = [i[3] for i in best_result_matrix]
Y = [i[4] for i in best_result_matrix]
Z = [i[5] for i in best_result_matrix]
ax.scatter(X, Y, Z)
ax.set_xlabel(u'成本')
ax.set_ylabel(u'服务质量')
ax.set_zlabel(u'碳排放')
plt.show()
plt.close()
...
#动态成本迭代图
plt.figure()
X = [i[0] for i in best_fee]
Y = [i[1] for i in best_fee]
plt.plot(X, Y)
ax = plt.gca()
ax.set_xlabel('迭代次数')
ax.set_ylabel('成本')
...
#车辆路径图
plt.figure()
plt.plot([i[1] for i in data], [i[2] for i in data], 'b.', marker=u'$\cdot$')
best_result_matrix_draw = filter(lambda x: x != '@', best_result_matrix[0][1])
best_result_matrix_draw = [i for i in best_result_matrix_draw]
for i in range(len(best_result_matrix_draw) - 1):
    plt.plot([data[best_result_matrix_draw[i]][1], data[best_result_matrix_draw[i +1]][1]], [data[best_result_matrix_draw[i]][2], data[best_result_matrix_draw[i +1]][2]], 'k')
ax = plt.gca()
